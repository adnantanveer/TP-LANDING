 ...
 * overlay folds away like a book page — pivoting from its bottom-right
 * corner and lifting diagonally toward the top-left, a real 3D rotation
 * (combined rotateX + rotateY around a corner-anchored transform-origin),
 * not a fade — to reveal the hero beneath. One continuous sequence
 * ("Phase 6"), not "loader finishes, then site appears".
 */
      ...
      {open && (
        ...
        >
          ...
          {/* page-shading: darkens toward the hinge-opposite corner
              (top-left), sells the "paper catching less light as it folds
              away" read */}
          <div
            className="pointer-events-none absolute inset-0"
            style={{ background: "linear-gradient(to top left, transparent 42%, rgba(0,0,0,0.45) 100%)" }}
          />
          ...
          <div
            className="pointer-events-none absolute inset-0"
            style={{ background: "linear-gradient(to top left, transparent 55%, rgba(255,255,255,0.06) 62%, transparent 70%)" }}
          />
        </motion.div>
      )}
    </AnimatePresence>

...
const fieldClass =
  "w-full rounded-md border border-border bg-background px-5 py-4 text-sm text-foreground placeholder:text-muted-foreground/70 outline-none transition-colors focus:border-primary";
...
export function ShellContactPage() {
  ...
  return (
    <main id="top" className="concept-shell relative">
      ...
      <section className="relative overflow-hidden border-b border-border bg-background pb-20 pt-32 md:pb-28 md:pt-40">
        <div className="relative mx-auto grid max-w-6xl gap-14 px-6 md:grid-cols-2 md:items-start">
          <div>
            <Reveal>
              ...
              <h1 className="mt-5 text-[clamp(1.9rem,4.6vw,3rem)] font-display uppercase leading-[0.95]">{contactInfo.heading}</h1>
            </Reveal>
            <Reveal delay={0.1}>
          ...
          <Reveal delay={0.1}>
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-border bg-card p-6 sm:p-8">
                ...
                <button
                  ...
                  className="w-full rounded-md bg-primary px-8 py-4 text-sm font-medium text-primary-foreground shadow-[var(--shadow-ember)] transition-transform duration-300 hover:scale-[1.02] disabled:opacity-60 sm:w-auto"
                >

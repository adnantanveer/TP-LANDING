...
export function SurgeTestimonials() {
  ...
  return (
    <section id="testimonials" ref={ref} className="relative h-[260vh] bg-background">
      <div className="sticky top-0 flex h-screen flex-col items-center justify-center overflow-hidden px-6">
        <div className="relative h-[44vh] w-full max-w-2xl">
...
function Quote({
  ...
  return (
    <motion.figure style={{ opacity, scale }} className="absolute inset-0 flex flex-col items-center justify-center rounded-[2.5rem] border border-border bg-card p-12 text-center">
      <blockquote>
        <p className="text-[clamp(1.3rem,2.8vw,2.1rem)] font-medium leading-[1.35] text-foreground">&ldquo;{quote}&rdquo;</p>
      </blockquote>
      <figcaption className="mt-7 text-sm text-primary">
        {name}, {org}
      </figcaption>
    </motion.figure>
  );
...
function Dot({ index, total, progress }: { index: number; total: number; progress: MotionValue<number> }) {
  ...
  return <motion.span style={{ opacity }} className="h-2 w-2 rounded-full bg-primary" />;
}
